...
def resource_number(value: str) -> int | None:
    match = re.search(r"(\d+)$", str(value))
    return int(match.group(1)) if match else None


def draw_resources(draw: dict[str, Any]) -> dict[str, str]:
    result = {}
...
def load_world_from_export(event_path: Path) -> dict[str, Any] | None:
...
def normalized_draws(manifest_path: Path, batch_root: Path | None = None) -> list[dict[str, Any]]:
...
def resource_distance(left: list[int], right: list[int]) -> int:
    if not left or not right:
        return 2**31 - 1
    return min(abs(a - b) for a in left for b in right)


def material_signature(resources: dict[str, str]) -> str:
...
def cluster_draws(
    ...
    resource_gap: int = 512,
    min_total_indices: int = 300,
) -> list[dict[str, Any]]:
