...
RGBA8_FORMATS = {27, 28, 29, 87, 88, 91, 93}
...
def parse_dds(path):
    ...
    if fourcc == b"DX10":
        ...
        if fourcc in legacy_formats:
            ...
            pixel_flags = int.from_bytes(data[80:84], "little")
            ...
            legacy_rgb_formats = {
                (32, 0x000000FF, 0x0000FF00, 0x00FF0000, 0xFF000000): 28,
                (32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000): 87,
                (32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0x00000000): 88,
            }
            dxgi_format = legacy_rgb_formats.get((rgb_bits,) + masks)
            if not (pixel_flags & 0x40) or dxgi_format is None:
                raise ValueError(
                    "{} uses unsupported legacy DDS pixel format flags=0x{:X} bits={} masks={}".format(
                        path, pixel_flags, rgb_bits,
                        tuple("0x{:08X}".format(value) for value in masks),
                    )
                )
        else:
