...
def bulk_size_table_matches(index_data, blocks):
    ...
    if not blocks or required > len(index_data):
        return matches
    signature = blocks[0]["compressed_size"].to_bytes(2, "little")
    offset = 0
    while True:
        offset = index_data.find(signature, offset)
        if offset < 0:
            break
        if offset + required > len(index_data):
            break
        if all(
            int.from_bytes(
...
def rebuild_container(container, payload):
    ...
    for block in container["payload_blocks"]:
        ...
        full_block_data = block_data + block["preserved_suffix"]
        if full_block_data == block["data"]:
            compressed = block["compressed_data"]
        else:
            compressed = compress_raw(full_block_data)
