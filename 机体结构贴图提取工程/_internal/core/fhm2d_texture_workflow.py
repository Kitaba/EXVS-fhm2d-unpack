...
def convert_dds_to_png(texconv, dds_paths, png_dir):
    ...
    for start in range(0, len(dds_paths), batch_size):
        ...
        for dds_path in batch:
            png_path = png_dir / dds_path.with_suffix(".png").name
            with Image.open(png_path) as opened:
                if opened.mode != "RGBA":
                    rgba = opened.convert("RGBA")
                    rgba.save(png_path, format="PNG", compress_level=9)
            retag_png_srgb(png_path)
        ...
        rows.append(
            {
                ...
                "mip_count": int(texture.get("mip_count", 1)),
                "allocation_size": int(
                    texture.get("allocation_size", texture["data_size"])
                ),
...
def convert_changed_pngs(texconv, project_dir, changed_rows, dds_dir):
    ...
    def encode(row):
        ...
        if int(row.get("mip_count", 1)) > 1:
            normalized_path.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(png_path, normalized_path)
            normalized_pixels = 0
        else:
        ...
        elif texture_format == FHM2D_BC7_SRGB_FORMAT:
            texconv_format = "BC7_UNORM_SRGB"
            expected_dxgi = 99
            compression_args = ["-bc", "q"]
        elif texture_format == FHM2D_BC3_FORMAT:
            texconv_format = "BC3_UNORM"
            expected_dxgi = 77
            compression_args = []
        elif texture_format == FHM2D_BC3_SRGB_FORMAT:
            texconv_format = "BC3_UNORM_SRGB"
            expected_dxgi = 78
            compression_args = []
        elif texture_format == FHM2D_BC4_FORMAT:
            texconv_format = "BC4_UNORM"
            expected_dxgi = 80
            compression_args = []
        ...
        if dds["mip_count"] != int(row.get("mip_count", 1)) or dds["array_size"] != 1:
            raise ValueError(
...
def build_project(
    ...
    for status in changed_rows:
        ...
        item = encoded[key]
        encoded_pixels = item["dds"]["pixel_data"]
        expected_size = int(texture.get("allocation_size", texture["data_size"]))
        if len(encoded_pixels) != expected_size:
            raise ValueError(
                f"encoded data size {len(encoded_pixels)} differs from expected "
        ...
        payload[start:end] = encoded_pixels
        modified_report.append(
            {
                ...
                "modified_pixel_sha256": hashlib.sha256(encoded_pixels).hexdigest(),
    ...
    report = {
        ...
        "texconv": {
            ...
            "sha256": sha256_file(texconv),
            "encoding": (
                "Source texture format and mip count, DX10 header, CPU, "
                "single process"
            ),
        },
    }
