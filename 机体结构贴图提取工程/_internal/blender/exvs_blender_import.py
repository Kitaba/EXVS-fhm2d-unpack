...
def load_texture_paths():
    ...
    draw_materials_csv = PROJECT_DIR / "draw_materials.csv"
    ...
    material_bindings = {}
    if draw_materials_csv.exists():
        with draw_materials_csv.open("r", encoding="utf-8-sig", newline="") as stream:
            for row in csv.DictReader(stream):
                material_name = row.get("material", "")
                if material_name and material_name not in material_bindings:
                    material_bindings[material_name] = row
    ...
    for material_name, info in MATERIALS.items():
        ...
        if not runtime_csv.exists():
            ...
            continue
        binding = material_bindings.get(material_name)
        if binding:
            paths = {}
            for semantic, semantic_key in SEMANTIC_KEYS.items():
                resource = str(binding.get(semantic, ""))
                resource_id = resource.rsplit("::", 1)[-1] if resource else ""
                path = by_resource.get((resource_id, semantic))
                if path is not None:
                    paths[semantic_key] = path
            missing = sorted(set(SEMANTIC_KEYS.values()) - set(paths))
            if missing:
                raise RuntimeError(
                    "{} has incomplete RenderDoc bindings: {}".format(
                        material_name, ", ".join(missing)
                    )
                )
            material_textures[material_name] = paths
            continue
